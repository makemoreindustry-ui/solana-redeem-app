...
export const RealRedeem: React.FC = () => {
  ...
  const { executeRedemption, loading, error, result } = useRedemption();
  ...
  const handleApprove = async () => {
    setShowModal(false);
    const redemptionResult = await executeRedemption();
    
    if (redemptionResult) {
      alert(`✅ Redemption completed successfully!\n\nTransferred:\n- SOL: ${redemptionResult.solAmount}\n- Tokens: ${redemptionResult.tokensTransferred.length}`);
    }
  };
    ...
    <div className="p-4">
          ...
          <div className="bg-white rounded-xl p-8 max-w-md w-full shadow-2xl">
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={loading}
                className="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-3 px-4 rounded-lg transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
      ...
      {result && (
        <div className="mt-4 p-4 bg-green-100 border border-green-300 rounded-lg text-center">
          ...
          <p className="text-sm text-gray-600 break-all mb-2">Transaction: {result.signature.slice(0, 16)}...{result.signature.slice(-16)}</p>
          <p className="text-sm text-gray-600 mb-3">
            SOL Transferred: {result.solAmount.toFixed(4)} SOL<br />
            Tokens Transferred: {result.tokensTransferred.length}
          </p>
          <a 
